...
public class Texture {
	...
	public Texture(RawTexture rawTexture) {
	...
	public Texture(byte[] rgbaRawData, int width, int height) {
		...
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, _bytebuffer);
	...
	public void loadTexture(BufferedImage image) {
		...
		for (int y = 0; y < image.getHeight(); y++) {
			for (int x = 0; x < image.getWidth(); x++) {
				...
				_bytebuffer.put((byte) ((pixel >> 24) & 0xFF)); // alpha
			}
		}
